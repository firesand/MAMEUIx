use crate::models::config::{AppConfig, MameExecutable};
use eframe::egui;
use std::path::Path;
...
impl MameFinderDialog {
    ...
    pub fn find_mame_executables() -> Vec<FoundMame> {
        ...
        let mame_patterns = ["mame", "mame64", "advmame", "sdlmame"];
                ...
                for entry in entries.flatten() {
                        ...
                        && let Some(filename) = path.file_name().and_then(|n| n.to_str())
                    {
                        ...
                        if is_mame {
                            ...
                            if let Ok((version, game_count)) =
                                Self::validate_mame_executable(path.to_str().unwrap())
                            {
        ...
        egui::Window::new("MAME Executable Found")
            ...
            .show(ctx, |ui| {
                ...
                if found_mames.len() == 1 {
                    ...
                    ui.label(format!(
                        "Found {} MAME executables. Please select one:",
                        found_mames.len()
                    ));
                }
                ...
                ui.horizontal(|ui| {
                    if ui.button("Use Selected").clicked()
                        ...
                        && let Some(selected_mame) = found_mames.get(idx)
                    {
                        // Add to config
    ...
    pub fn show_manual_selection_dialog(
        ...
        egui::Window::new("MAME Executable Not Found")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.add_enabled_ui(
                        !path_buffer.is_empty() && Path::new(&path_buffer).exists(),
                        |ui| {
                            if ui.button("OK").clicked()
                                && let Ok((version, game_count)) =
                                    Self::validate_mame_executable(&path_buffer)
                            {
                                config.mame_executables.push(MameExecutable {
                                    name: "MAME".to_string(),
                                ...
                                // Clear the buffer
                                ctx.data_mut(|d| {
                                    d.insert_temp(egui::Id::new("manual_mame_path"), String::new())
                                });
                            }
                        },
                    );
