...
impl IconManager {
    ...
    pub fn init_default_icon(&mut self, ctx: &egui::Context, icon_size: u32) {
        ...
        self.default_icon_texture =
            Some(ctx.load_texture("default_icon", color_image, egui::TextureOptions::default()));
    }
    ...
    fn load_icon_from_file_threaded(
        ...
        if ico_path.exists() {
            ...
            if let Ok(ico_data) = std::fs::read(&ico_path) {
                ...
                if let Ok(image) =
                    image::load_from_memory_with_format(&ico_data, image::ImageFormat::Ico)
                {
                    // Convert to RGBA8
                    ...
                    let resized =
                        if rgba_image.width() != icon_size || rgba_image.height() != icon_size {
    ...
    fn initialize_thread_pool(&mut self, _config: &AppConfig) {
            ...
            .build()
        {
            Ok(pool) => {
                ...
                println!(
                    "Icon loading thread pool initialized with {} threads",
                    self.max_concurrent_loads
                );
            }
            Err(err) => {
                eprintln!("Failed to initialize icon loading thread pool: {err}");
            }
        }
            ...
            while let Ok(result) = rx.try_recv() {
                ...
                if let Some(load_time) = result.load_time {
                    self.performance_monitor
                        .record_load(load_time, result.width, result.success);
                }
                ...
                if let Some(icon_data) = result.icon_data {
                    ...
                    self.icon_info.insert(
                        result.rom_name.clone(),
                        IconInfo {
    ...
    pub fn cleanup_old_icons(&mut self) {
        ...
        if self.rom_icons.len() > self.max_cached_icons {
            let mut icon_ages: Vec<_> = self
                .icon_info
                .iter()
                .map(|(name, info)| (name.clone(), info.last_accessed))
                .collect();

            icon_ages.sort_by_key(|a| a.1);
    ...
    pub fn get_cache_stats(&self) -> (usize, usize, usize) {
        ...
        (self.rom_icons.len(), queue_size, self.icon_info.len())
    }
