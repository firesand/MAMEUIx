...
impl ArtworkLoader {
    ...
    pub fn load_artwork(
        ...
        let search_dir = match artwork_type {
            ArtworkType::Screenshot => config.snap_path.as_ref()?,
    ...
    fn load_from_directory(
        ...
        if zip_path.exists()
            && let Some(image_data) = self.extract_from_zip(&zip_path, game_name)
        {
        ...
        for ext in &["png", "jpg", "jpeg", "bmp", "gif"] {
            ...
            if image_path.exists()
                && let Ok(image_data) = std::fs::read(&image_path)
            {
                return self.create_texture(ctx, &image_data, game_name);
            }
        }
        ...
        if artwork_type == ArtworkType::Screenshot {
            ...
            if game_subdir.exists() && game_subdir.is_dir() {
                ...
                if let Ok(entries) = std::fs::read_dir(&game_subdir) {
                    ...
                    for entry in entries.flatten() {
                        if let Some(file_name) = entry.file_name().to_str() {
                            ...
                            if file_name.ends_with(".png")
                                && let Some(num_part) = file_name.strip_suffix(".png")
                            {
                    ...
                    numbered_files.sort_by_key(|a| std::cmp::Reverse(a.0));

                    if let Some((_num, path)) = numbered_files.first()
