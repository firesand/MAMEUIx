...
impl CategoryManager {
    ...
    pub fn get_categories_by_letter(&self) -> Vec<(char, Vec<&CatverCategory>)> {
        let mut result: Vec<(char, Vec<&CatverCategory>)> = self
    ...
    pub fn load_from_catver_map(&mut self, categories: &HashMap<String, String>) {
        ...
        for (category_name, game_count) in category_counts {
            ...
            self.categories
                .insert(category_name.clone(), catver_category);
...
impl FilterSettings {
    ...
    pub fn has_active_filters(&self) -> bool {
        ...
        let status_active =
            !(self.status_filters.show_working && self.status_filters.show_not_working);
    ...
    pub fn count_active_filters(&self) -> usize {
        ...
        {
            if self.availability_filters.show_available {
                count += 1;
            }
            if self.availability_filters.show_unavailable {
                count += 1;
            }
        }

        if !(self.status_filters.show_working && self.status_filters.show_not_working) {
            if self.status_filters.show_working {
                count += 1;
            }
            if self.status_filters.show_not_working {
                count += 1;
            }
        }

        if self.other_filters.show_favorites {
            count += 1;
        }
        if self.other_filters.show_parents_only {
            count += 1;
        }
        if self.other_filters.show_chd_games {
            count += 1;
        }

        if !self.cpu_filter.is_empty() {
            count += 1;
        }
        if !self.device_filter.is_empty() {
            count += 1;
        }
        if !self.sound_filter.is_empty() {
            count += 1;
        }
