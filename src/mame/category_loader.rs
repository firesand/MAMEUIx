...
impl CategoryLoader {
    ...
    pub fn new(catver_path: &Path) -> Result<Self, std::io::Error> {
        ...
        if catver_path.exists() {
            ...
            for line in reader.lines() {
                ...
                if in_category_section && let Some(equals_pos) = trimmed.find('=') {
    ...
    pub fn get_category_with_parent(
        &self,
        game_name: &str,
        ...
        if let Some(parent) = parent_name
            && let Some(category) = self.get_category(parent)
        {
            return Some(category);
        }
...
mod tests {
    ...
    fn test_missing_file() {
        let err = CategoryLoader::new(Path::new("/nonexistent/catver.ini")).unwrap_err();
