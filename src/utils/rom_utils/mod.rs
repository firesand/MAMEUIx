...
impl RomLoader {
    ...
    pub fn load_roms(&self, metadata: HashMap<String, Game>) -> Vec<Game> {
        println!(
            "Starting ROM scan with {} games in metadata",
            metadata.len()
        );
    ...
    pub fn load_roms_with_progress<F>(
        &self,
        metadata: HashMap<String, Game>,
        ...
        F: FnMut(usize, usize) + Send,
    {
        ...
        // Buat lookup map, dipisah supaya status CHD tetap akurat.
        let mut available_roms: HashMap<String, PathBuf> = HashMap::new();
        let mut available_chds: HashMap<String, PathBuf> = HashMap::new();
        for (rom_path, rom_name) in rom_files {
            if rom_path
        ...
        for rom_dir in &self.rom_dirs {
                ...
                .filter_map(|e| e.ok())
            {
                ...
                if let Some(extension) = path.extension()
                    ...
                    && rom_extensions.contains(&ext_str.to_lowercase().as_str())
                {
