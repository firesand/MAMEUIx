    ...
    pub fn load_shader(&mut self, shader_name: &str) -> Result<ShaderInfo> {
        ...
        if vert_path.exists() {
            shader_info.vertex_shader =
                Some(fs::read_to_string(&vert_path).context("Failed to read vertex shader")?);
        }
        ...
        if frag_path.exists() {
            shader_info.fragment_shader =
                Some(fs::read_to_string(&frag_path).context("Failed to read fragment shader")?);
        }
        ...
        if params_path.exists() {
            let params_data =
                fs::read_to_string(&params_path).context("Failed to read shader parameters")?;
            shader_info.parameters =
                serde_json::from_str(&params_data).context("Failed to parse shader parameters")?;
        }
        ...
        self.compiled_shaders
            .insert(shader_name.to_string(), shader_info.clone());
    ...
    pub fn create_shader_template(&self, shader_name: &str, template_type: &str) -> Result<()> {
        ...
        match template_type {
            "crt-geom" => {
                ...
                fs::write(
                    shader_dir.join("parameters.json"),
