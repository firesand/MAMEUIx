...
impl HardwareFilter {
    ...
    pub fn game_uses_cpu(&self, game_name: &str, cpu_search: &str) -> bool {
        ...
        if let Some(cpus) = self.game_to_cpus.get(game_name) {
            cpus.iter()
                .any(|cpu| cpu.to_lowercase().contains(&cpu_search_lower))
        } else {
            false
        }
    }
    ...
    pub fn game_uses_device(&self, game_name: &str, device_search: &str) -> bool {
        ...
        if let Some(devices) = self.game_to_devices.get(game_name) {
            devices
                .iter()
                .any(|device| device.to_lowercase().contains(&device_search_lower))
        } else {
            false
        }
    }
    ...
    pub fn game_uses_sound(&self, game_name: &str, sound_search: &str) -> bool {
        ...
        if let Some(sounds) = self.game_to_sounds.get(game_name) {
            sounds
                .iter()
                .any(|sound| sound.to_lowercase().contains(&sound_search_lower))
        } else {
            false
        }
    }

    /// Returns true if any hardware INI data was loaded.
    pub fn is_loaded(&self) -> bool {
        !self.cpu_to_games.is_empty()
            || !self.device_to_games.is_empty()
            || !self.sound_to_games.is_empty()
    }

    pub fn cpu_count(&self) -> usize {
        self.cpu_to_games.len()
    }

    pub fn device_count(&self) -> usize {
        self.device_to_games.len()
    }

    pub fn sound_count(&self) -> usize {
        self.sound_to_games.len()
    }

    /// Apply sidebar hardware filter fields (empty field = no constraint).
    pub fn matches_hardware_filters(
        &self,
        game_name: &str,
        cpu_filter: &str,
        device_filter: &str,
        ...
        if !cpu_filter.is_empty() && !self.game_uses_cpu(game_name, cpu_filter) {
            return false;
        }
        if !device_filter.is_empty() && !self.game_uses_device(game_name, device_filter) {
            return false;
        }
        if !sound_filter.is_empty() && !self.game_uses_sound(game_name, sound_filter) {
            return false;
        }
        true
    }

    /// Load CPU/device/sound INI files from the configured MAME INI directory.
    pub fn load_from_config(config: &crate::models::AppConfig) -> Option<Self> {
        let ini_dir = config.ini_path.as_deref()?;
        let cpu = ini_dir.join("cpu.ini");
        let device = ini_dir.join("device.ini");
        let sound = ini_dir.join("sound.ini");

        let cpu_path = cpu.exists().then_some(cpu);
        let device_path = device.exists().then_some(device);
        let sound_path = sound.exists().then_some(sound);

        if cpu_path.is_none() && device_path.is_none() && sound_path.is_none() {
            return None;
        }

        match Self::load_from_ini_files(
            cpu_path.as_deref(),
            device_path.as_deref(),
            sound_path.as_deref(),
        ) {
            Ok(filter) if filter.is_loaded() => Some(filter),
            Ok(_) => None,
            Err(err) => {
                eprintln!("Failed to load hardware INI files: {err}");
                None
            }
        }
    }
...
mod tests {
    ...
    fn test_load_cpu_ini() {
        ...
        filter
            .load_ini_file(temp_file.path(), HardwareType::Cpu)
            .unwrap();
