...
if command -v podman >/dev/null 2>&1; then
    ...
    print_error "  gh workflow run appimage.yml -f build_tag=v${VERSION} -f upload_to_release=false"
    exit 1
fi
...
"${RUNTIME}" run --rm \
    ...
    -e CARGO_TARGET_DIR=/tmp/mameuix-target \
    "${IMAGE}" \
    bash -lc '
set -euo pipefail
...
apt-get install -y -qq \
    ...
    libwayland-dev libssl-dev libasound2-dev
