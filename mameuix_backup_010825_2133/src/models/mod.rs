...
pub enum FilterCategory {
    ...
    Available,
    Missing,
    Unavailable, // New: Games that exist but can't be run (vs Missing: ROMs not found)
    Clones,
    NonClones,
    Favorites,
    Working,
...
// New hierarchical filter system
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum FilterCategoryGroup {
    All,
    Availability(FilterCategory),
    Status(FilterCategory),
    Others(FilterCategory),
}
...
impl FilterCategoryGroup {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ...
            FilterCategoryGroup::Availability(FilterCategory::Available) => "Available",
            FilterCategoryGroup::Availability(FilterCategory::Missing) => "Missing",
            FilterCategoryGroup::Status(FilterCategory::Working) => "Working",
            FilterCategoryGroup::Status(FilterCategory::NotWorking) => "Not Working",
            FilterCategoryGroup::Others(FilterCategory::Favorites) => "Favorites",
            FilterCategoryGroup::Others(FilterCategory::Parents) => "Parent ROMs",
            FilterCategoryGroup::Others(FilterCategory::ChdGames) => "CHD Games",
            _ => "Unknown",
        }
    }
    ...
    pub fn group_name(&self) -> &'static str {
        match self {
            ...
            FilterCategoryGroup::Availability(_) => "AVAILABILITY",
            FilterCategoryGroup::Status(_) => "STATUS",
            FilterCategoryGroup::Others(_) => "OTHERS",
        }
    }
}
